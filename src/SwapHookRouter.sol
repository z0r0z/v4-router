...
import {CurrencyLibrary, Currency} from "@v4/src/types/Currency.sol";
...
import {PoolTestBase} from "@v4/src/test/PoolTestBase.sol";
import {CurrencySettler} from "@v4/test/utils/CurrencySettler.sol";

contract SwapHookRouter is PoolTestBase {
    ...
    function unlockCallback(bytes calldata rawData) external returns (bytes memory) {
        ...
        if (deltaAfter0 > 0) {
            data.key.currency0.take(
                manager, data.sender, uint256(deltaAfter0), data.testSettings.takeClaims
            );
        }
        if (deltaAfter1 > 0) {
            data.key.currency1.take(
                manager, data.sender, uint256(deltaAfter1), data.testSettings.takeClaims
            );
        }
